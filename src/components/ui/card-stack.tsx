...
export type CardStackProps<T extends CardStackItem> = {
  ...
  /** Override stage height (e.g. for mobile to reduce gap above cards) */
  stageHeight?: number;
  className?: string;
...
export function CardStack<T extends CardStackItem>({
  ...
  const stageHeight = stageHeightProp ?? Math.max(380, cardHeight + 80);
  ...
  return (
    ...
    >
      ...
      {/* Prev/next arrows - below cards on mobile for minimal gap */}
      {showArrows && len > 1 ? (
        <div className="mt-0 sm:mt-2 flex items-center justify-center gap-2 sm:gap-4">
          <button
            type="button"
            onClick={prev}
            aria-label="Previous card"
            className={cn(
              "flex items-center justify-center min-h-[44px] min-w-[44px] rounded-full border-2 border-foreground/20 text-foreground/70 hover:bg-foreground/10 hover:text-foreground hover:border-foreground/40 transition-colors",
      ...
      {showProgressBar && autoAdvance && !reduceMotion && len > 1 ? (
        <div
          key={progressKey}
          className="mt-4 h-1 w-full max-w-xs mx-auto rounded-full bg-foreground/10 overflow-hidden"
          aria-hidden
        >
          <motion.div
            ...
            {items.map((it, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "rounded-full transition shrink-0 flex items-center justify-center",
                    "h-2.5 w-2.5 sm:h-2 sm:w-2 p-4 sm:p-0 min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0",
                    on
                      ? "bg-foreground"
                      : "bg-foreground/30 hover:bg-foreground/50 active:bg-foreground/40",
          ...
          {activeItem.href ? (
            isExternalHref(activeItem.href) ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center min-h-[44px] min-w-[44px] rounded-lg text-muted-foreground hover:text-foreground hover:bg-foreground/5 transition"
